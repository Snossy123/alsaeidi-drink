    ...
    <div className="space-y-8 p-8" dir="rtl">
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-6">
        {[
          ...
          { label: "نوع الفاتورة", value: purchaseInvoiceTypeLabels[type], icon: FileText, color: isGeneral ? "text-purple-500" : "text-amber-500" },
      ...
      <div className="bg-white dark:bg-slate-950 rounded-[2.5rem] overflow-hidden border border-slate-100 dark:border-slate-800 shadow-xl">
        <div className="p-6 border-b border-slate-50 dark:border-slate-800/50 flex justify-between items-center bg-slate-50/50 dark:bg-slate-950/50">
          <h3 className="text-lg font-black text-slate-800 dark:text-white flex items-center gap-2">
            <Package className={`w-5 h-5 ${isGeneral ? "text-purple-500" : "text-amber-500"}`} />
            {isGeneral ? "المنتجات المضافة في هذه الفاتورة" : "بنود المصروفات"}
          </h3>
        </div>
          ...
          <Table>
            <TableHeader className="bg-slate-50/50 dark:bg-slate-900/50">
              <TableRow className="border-slate-100 dark:border-slate-800">
                <TableHead className="text-right py-5 px-6 font-black text-slate-500 text-xs uppercase">
                  {isGeneral ? "المنتج والباركود" : "الوصف"}
                </TableHead>
                {isGeneral && (
                  <>
                ...
                {!isGeneral && (
                  <TableHead className="text-right py-5 px-4 font-black text-slate-500 text-xs uppercase">المبلغ</TableHead>
                )}
            ...
            <TableBody>
              {invoice.items.map((item, index) => (
                <TableRow key={index} className="border-slate-50 dark:border-slate-800/50 hover:bg-slate-50/50 dark:hover:bg-slate-900/30 transition-colors">
                  ...
                  {isGeneral && (
                    <>
                      ...
                      <TableCell className="py-5 px-4 text-center">
                        <Badge variant="outline" className="rounded-lg px-2 py-0.5 border-slate-200 dark:border-slate-800 font-black">
                          {item.quantity}
                        </Badge>
                      </TableCell>
                    </>
                  ...
                  {!isGeneral && (
                    <TableCell className="py-5 px-4 font-bold text-amber-600">{Number(item.purchase_price).toFixed(2)} ج</TableCell>
                  )}
                  <TableCell className="py-5 px-6 text-left font-black text-slate-800 dark:text-white">
                    ...
                    <span className="text-[10px] text-slate-400">ج</span>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
