...
function getGridConfig(width: number): GridColumnsConfig {
  if (width < 640) return { cols: 1, itemsPerPage: 4 };
  if (width < 1024) return { cols: 2, itemsPerPage: 8 };
  if (width < 1536) return { cols: 3, itemsPerPage: 12 };
  if (width < 1920) return { cols: 4, itemsPerPage: 16 };
  return { cols: 5, itemsPerPage: 20 };
}
